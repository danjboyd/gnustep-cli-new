...
Invoke-QualificationCommand -Command 'gnustep build --clean' -WorkingDirectory $smokeRoot -LogPath (Join-Path $OutputDir 'smoke-build.log')
...
if (-not $SkipGorm) {
  ...
  Invoke-QualificationCommand -Command 'gnustep build --clean' -WorkingDirectory $GormRoot -LogPath (Join-Path $OutputDir 'gorm-build.log')
