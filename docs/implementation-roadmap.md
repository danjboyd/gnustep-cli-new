...
- Phase 12 hardening status is now machine-checkable through `scripts/internal/build_infra.py --json phase12-production-hardening-status`. The gate composes explicit release trust roots, controlled-release validation, release key-rotation drills, and Phase 26 host-backed smoke evidence. A synthetic complete-evidence fixture can pass the gate in regression coverage; the April 27, 2026 local production-like run satisfies the host-backed Tier 1 smoke evidence input with the OpenBSD and Windows reports and verifies the refreshed dogfood release directory with explicit release and package-index trust roots. The real phase remains blocked on CI-held production keys or an external signing service, controlled package/release publication from production build lanes, and clean hosted reruns that do not depend on operator-local artifacts.
...
- Regression coverage now includes update-available planning, downgrade rejection, expired metadata rejection, frozen older-manifest rejection, revoked selected-artifact rejection, needs-repair upgrade rejection, checksum rollback, stale transaction recovery, local artifact path staging, archive layout preservation, runtime-bundle double-wrap prevention, conflict rejection, smoke-validated pointer activation, explicit rollback, versioned-release snapshot creation, previous-release preservation, default `update all` planning, combined `update all --check`, `update all --yes` across CLI/toolchain and package updates, `update cli --yes`, package-update rollback, update usage-error JSON, package-update human output, and built-executable `gnustep update` smoke tests.
- Phase 13 is complete for native dogfood: `gnustep update` owns check/apply UX for CLI, package, and default all-scope updates; rollback, stale transaction recovery, downgrade rejection, expired metadata rejection, revoked artifact rejection, and package-update rollback are covered. Remaining Phase 13 hardening is repeating old-to-new dogfood and `update all --yes` evidence from controlled release infrastructure against real published update-capable releases and production-held signing material.
...
- Phase 13 hardening status is now machine-checkable through `scripts/internal/build_infra.py --json phase13-update-hardening-status`. The gate requires live old-to-new smoke reports containing `self-update-cli-only`, production-like `update all --yes` evidence, and a release-key mismatch/rotation drill. The April 27, 2026 local production-like run satisfies the old-to-new/self-update smoke evidence, signed metadata key-mismatch drill, and `update all --yes` evidence inputs using the refreshed dogfood release directory and evidence at `.artifacts/phase13-local-production-like/evidence/update-all-production-like.json`. That run exposed and fixed a native `update all` defect where package state could be lost across the CLI/toolchain update and raw package failures were not enforced by the evidence gate. The real phase remains blocked on repeating the same path from CI/release infrastructure with production-held signing material and clean Tier 1 hosted smoke runs.
