...
- `prepare-github-release` still generates the host-validation plan but does not
  ...
  availability
- `scripts/internal/build_infra.py --json package-artifact-build-plan --packages-dir packages` now exposes the reviewed package publication inputs explicitly, and `package-artifact-publication-gate` fails release publication while source or artifact digest placeholders remain. `tools-xctest-release-gate` is the stricter Phase 24 lifecycle gate: Debian amd64, Ubuntu amd64, Linux arm64, OpenBSD amd64, OpenBSD arm64, and Windows/MSYS2 now have patched, published-in-metadata, validated artifacts with native install/smoke/minimal-bundle/remove evidence. OpenBSD arm64 validation uses the accepted `openbsd-7.8-arm64` profile with `/home/danboyd/.ssh/otvm/id_rsa.pub`; managed OpenBSD arm64 toolchain publication remains deferred until a source-built toolchain artifact exists.
- production signing policy is documented in `docs/security-production-signing.md`; release and package-index signing keys must remain distinct, release publication now requires package-index signing material, production gates must verify against externally pinned trust roots, and hosted package-index assets must include a bundled `package-index-signing-public.pem` that verifies the signed package index and provenance
