...
class BuildInfraTests(unittest.TestCase):
    ...
    def test_assemble_linux_toolchain_artifact(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertIn(
                '-L"$SYSROOT_LIBRARY_DIR" -L"$SYSROOT_RUNTIME_LIBRARY_DIR"',
                (assembled / "System" / "Tools" / "clang").read_text(),
            )
            self.assertTrue(
            ...
            self.assertIn(
                '--sysroot="$SYSROOT" -L"$GCC_RUNTIME_DIR"',
    ...
    def test_bundle_full_cli(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertNotIn("GNUSTEP_SYSTEM_ROOT", launcher)
            self.assertNotIn("GNUSTEP_LOCAL_ROOT", launcher)
